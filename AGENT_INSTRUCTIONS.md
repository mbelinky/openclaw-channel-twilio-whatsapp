...
1. **A running OpenClaw gateway, version `>= 2026.5.26`.** Earlier 2026.x gateways have an `EACCES mkdir '/home/openclaw'` bug in the gateway core that silently breaks outbound media (text replies work; images don't arrive). If the user can only run an older gateway, the plugin will still load but warn them: media is broken until they upgrade.
2. **Plugin version `>= 2.1.6` (install `@latest`).** Earlier plugin versions registered the media-serve route without prefix matching, so Twilio's CDN 404s on every staged image URL. There is no reason a new install should pin an older version — always use `@latest`.
...
| Twilio WhatsApp sender number | E.164, e.g. `+14155550000` (Sandbox: `+14155238886`) | `fromNumber` |
| Allowed sender phone numbers | E.164 list, e.g. `["+14155551234"]` | `allowFrom` |
...
- **Pin `<version>` to a tag `>= 2026.5.26`** in line with the prerequisite above.
...
- `dmPolicy: "open"` is allowed but strongly discouraged — anyone with the bot's number can talk to the agent and rack up Twilio charges. Default to `"allowlist"`.
...
2. **Inbound test:** from a phone number in `allowFrom`, WhatsApp the `fromNumber`. The agent should reply.

3. **Outbound media test:** ask the agent to send the user a file or image. The recipient should see the attachment, not just text. If text arrives and the image doesn't, the prerequisites weren't met — re-check gateway version (`>= 2026.5.26`) and plugin version (`>= 2.1.6`).
...
| Inbound messages get 403 only from one number | Number not in `allowFrom`, or has `whatsapp:` prefix in config | Add as E.164 without prefix |
| Text replies arrive but images don't | Gateway `< 2026.5.26` (mkdir EACCES) OR plugin `< 2.1.6` (media-route 404) | Upgrade whichever is behind; both are preconditions |
...
- **No threaded replies.**
- **No multiple Twilio accounts in one OpenClaw instance.** Single account only in current version.

## What to tell the user about cost

Twilio bills per WhatsApp message. Pricing varies by destination country and conversation type (utility / marketing / service). Point the user at https://www.twilio.com/en-us/whatsapp/pricing before they enable `dmPolicy: "open"` or before any production rollout.
